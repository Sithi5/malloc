...
static size_t show_tiny_zone() {
    write(1, "TINY : ", 8);
    ft_write_hex_pointer(g_zone.tiny);
    write(1, "\n", 1);
    return print_zone(g_zone.tiny);
}

static size_t show_small_zone() {
    write(1, "SMALL : ", 9);
    ft_write_hex_pointer(g_zone.small);
    write(1, "\n", 1);
    return print_zone(g_zone.small);
}

static size_t show_large_zone() {
    write(1, "LARGE : ", 9);
    ft_write_hex_pointer(g_zone.large);
    write(1, "\n", 1);
    return print_large_blocks(g_zone.large);
}
...
void show_alloc_mem() {
    ...
    total = 0;
    if (!g_zone.tiny && !g_zone.small && !g_zone.large) {
